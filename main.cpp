...
L atom(const char* s) {
	...
	if (i == hp && (hp += strlen(strcpy(A + i, s)) + 1) > sp << 3)
		abort();
	return box(ATOM, i);
}
...
L closure(L v, L x, L e) {
	return box(CLOS, ord(pair(v, x, equ(e, env) ?
		nil :
		e)));
}
...
L f_div(L t, L* e) {
	L n = car(t = evlis(t, *e));
	while (!_not(t = cdr(t))) n /= car(t); return num(n);
}
...
{"or",    f_or,    0},{"and",   f_and,   0},{"_not", f__not, 0},{"cond",f_cond,1},{"if", f_if, 1},{"let*",f_leta,1},
